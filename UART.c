...
void UART_Init(void){
  ...
  UART1_FBRD_R = 0;                     // FBRD = int(0 * 64 + 0.5) = 0
  ...
  GPIO_PORTC_AMSEL_R &= ~0x30;          // disable analog functionality on PA
	UART1_IM_R |= 0x10;
	UART1_IFLS_R &= ~0x38;
	UART1_IFLS_R |= 0x10;									// Enable interrupts on receiver fifo at half full
	NVIC_PRI1_R &= ~0x00700000;							// Make UART1 interrupt highest priority
	NVIC_EN0_R |= 0x40;
}
...
void UART1_Handler(){
	...
	// Check RXFE bit in UART1_FR_R
	while ((UART1_FR_R & 0x10) == 0) {
		int32_t data = UART1_DR_R & 0x0FF;
		int8_t status = fifo_Put(data);
		if (status != 1){ error_count++;}
	}
	RxCounter++;
	UART1_ICR_R = 0x10;
	//GPIO_PORTF_DATA_R ^= 0x08;
